...
import { LogOut, Plus, Trash2, User, Coffee, Settings, Moon, Sun, LayoutDashboard, X as XIcon, ChevronDown } from 'lucide-react';
...
export default function RestaurantVendorUI({ 
  ...
  const [history, setHistory] = useState([]); // Session History Only
  ...
  const handleAdminAddProduct = async () => { await fetch(`${API_URL}/products`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(newItem) }); setNewItem({ name: '', price: '', category: '' }); setIsAddingItem(false); setIsCreatingCategory(false); refreshProducts(); };
  ...
  const handleAdminAddUser = async () => { if(!newUser.username || !newUser.email || !newUser.password) return alert("Fill all fields"); await fetch(`${API_URL}/auth/signup`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(newUser) }); setNewUser({ username: '', email: '', password: '', role: 'cashier' }); refreshUsers(); };
  ...
  const finalizeOrder = async (paymentData) => {
    ...
    try {
      ...
      if (method === 'upi' && result.upi?.qr) {
          ...
          // Add to LOCAL UI Orders (Optimistic)
          const newOrder = { 
          ...
          // Add to LOCAL Session History (Since DB history is off)
          setHistory(prev => [{ ...newOrder, status: 'completed', completedAt: Date.now() }, ...prev]);
  ...
  // ✅ REVERTED: Simple DELETE from DB
  ...
  return (
    <div className={`h-screen overflow-hidden flex flex-col ${isDarkMode ? 'bg-slate-900' : 'bg-slate-50'}`}>
      <div className="flex-1 overflow-hidden relative">
          ...
          <AdminSettingsModal open={settingsOpen} onClose={() => setSettingsOpen(false)} API_URL={API_URL} />
          <POSView menu={menu} categories={categories} cart={cart} orders={orders} selectedCategory={selectedCategory} setSelectedCategory={setSelectedCategory} availableTokens={availableTokens} selectedToken={selectedToken} onSetToken={setSelectedToken} onAddToCart={addToCart} onRemoveFromCart={removeFromCart} onCheckout={handleCheckoutClick} onLogout={onLogout} userRole={userRole} isDarkMode={isDarkMode} onToggleTheme={onToggleTheme} discount={discount} setDiscount={setDiscount} taxRate={taxRate} onOpenSettings={() => setSettingsOpen(true)} onMarkReady={handleMarkReady} onCallCustomer={(t) => sendToDock(t)} onConnectDock={connectDock} dockConnected={dockConnected} />
      </div>
    </div>
  );
}
...
export default function SalesReport({ 
  ...
  useEffect(() => {
    const fetchHistory = async () => {
        ...
        try {
            ...
            let res = await fetch(`${API_URL}/orders/history?startDate=${startDate}&endDate=${endDate}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            
            if (!res.ok && res.status === 400) {
                // Fallback: try single date format
                res = await fetch(`${API_URL}/orders/history?date=${startDate}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            }
  ...
  const repairOrderData = (o) => {
    ...
    if (o.items && Array.isArray(o.items) && o.items.length > 0) {
      ...
      if (rawSubtotal > 0) {
          ...
          if (tax === 0) tax = taxableAmount * 0.05; 
          if (amount === 0) amount = Math.round(taxableAmount + tax);
      }
    }
  ...
  const exportData = () => {
    ...
    const headers = ["ID", "Date", "Time", "Items", "Total", "Method", "Status"];
    const rows = filteredOrders.map(o => [
        o.id, 
        new Date(o.activeDate).toLocaleDateString(),
            ...
            <div>
                ...
                <p className={`text-sm ${theme.text.secondary} flex items-center gap-2`}>
                    {dateRange.from === dateRange.to 
                      ? `Overview for ${dateRange.from}` 
                      : `Overview from ${dateRange.from} to ${dateRange.to}`
                    }
                ...
                <div className="h-40 flex gap-3 items-end min-w-[600px] md:min-w-0">
                    {chartData.data.map((val, idx) => (
                        <div key={idx} className="flex-1 max-w-[60px] flex flex-col justify-end items-center group relative h-full">
                            {val > 0 && (
                                <div className={`absolute bottom-full mb-2 px-2 py-1 rounded text-xs z-10 whitespace-nowrap ${isDarkMode ? 'bg-white text-black' : 'bg-black text-white'}`}>
                                    ₹{val}
                                </div>
                            )}
                            <div 
                            ...
                            />
                            <span className={`text-[10px] ${theme.text.secondary} mt-2 whitespace-nowrap overflow-hidden text-ellipsis`}>
                        ...
                        {filteredOrders.length === 0 ? (
                            ...
                            filteredOrders.map(o => (
                                <tr key={o.id} className={`${COMMON_STYLES.tableRow(isDarkMode)}`}>
                                    ...
                                    <td className="p-3">
                                        <div className="text-sm">{new Date(o.activeDate).toLocaleDateString('en-US', {month:'short', day:'numeric'})}</div>
                                        <div className={`text-xs ${theme.text.secondary}`}>{new Date(o.activeDate).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</div>
                                    </td>
                                    <td className="p-3">
                                    ...
                                    <td className={`p-3 text-right ${theme.text.secondary}`}>
                                        {o.tax > 0 ? Number(o.tax).toFixed(2) : '-'}
                                    </td>

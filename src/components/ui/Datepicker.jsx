...
export default function DatePicker({ value, onChange, isDarkMode }) {
  ...
  const handleDateClick = (day) => {
    ...
    if (!selection.from || (selection.from && selection.to)) {
      // Start new selection
      setSelection({ from: clickedDate, to: null });
      ...
      if (clickedDate < selection.from) {
        ...
        // Valid range - complete selection
        const newSelection = { from: selection.from, to: clickedDate };
        setSelection(newSelection);
        onChange({ 
          from: toDateStr(newSelection.from), 
          to: toDateStr(newSelection.to) 
        });
        setIsOpen(false);
      }
    }
  ...
  const formatDisplayDate = () => {
    if (value?.from) {
      const d1 = new Date(value.from).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
      if (!value.to || value.from === value.to) return d1;
      const d2 = new Date(value.to).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
      return `${d1} - ${d2}`;
    }
  ...
  return (
    <div ref={dropdownRef} className="relative">
      ...
      {isOpen && (
        <div 
          className={`absolute right-0 top-full mt-2 p-4 rounded-xl border shadow-2xl z-50 ${COMMON_STYLES.modal(isDarkMode)}`}
          style={{ width: '320px' }}
        >
              ...
              week.map((cell, dayIdx) => {
                ...
                if (cell.type !== 'current') {
                  return (
                    <div
                      key={key}
                      className={`h-9 flex items-center justify-center text-sm ${theme.text.muted} opacity-30`}
                    >
                      {cell.day}
                    </div>
                  );
                ...
                if (cell.isStart) {
                  ...
                  if (!selection.to) roundedClass = 'rounded-lg';
                } else if (cell.isEnd) {
                  bgClass = theme.button.primary;
                  textClass = selectedTextClass;
